  ...
  return (
    <div className="absolute inset-0 settings-overlay z-40 flex flex-col p-4 select-none animate-fadeIn transition-all duration-300">
      ...
      <div className="flex-1 overflow-y-auto pr-1 space-y-4 scrollbar-thin scrollbar-thumb-neutral-200 dark:scrollbar-thumb-neutral-800">
        ...
        {/* Section: System Preferences (Only in Electron Desktop Mode) */}
        {isElectron && (
          <div className="space-y-3 pt-3 border-t border-indigo-500/10 dark:border-white/5">
            <div className="text-[10px] font-extrabold text-slate-600 dark:text-neutral-400 tracking-wider uppercase select-none font-sans">
              💻 SYSTEM PREFERENCES
            ...
            <div className="flex items-center justify-between">
              ...
              {/* Upgraded Toggle Switch for Auto Launch */}
              <button
                type="button"
                onClick={() => onToggleLaunchOnStart?.(!launchOnStart)}
                className={`relative w-12 h-6 rounded-full transition-colors duration-200 cursor-pointer outline-none shrink-0 ${
                  launchOnStart 
                    ? 'bg-indigo-600 dark:bg-indigo-500' 
                    : 'bg-slate-200 dark:bg-neutral-800'
              ...
              >
                <div
                  className={`absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-white shadow-sm flex items-center justify-center text-[10px] transition-transform duration-200 ${
                    launchOnStart ? 'transform translate-x-6' : 'transform translate-x-0'
                  }`}
                >
                  {launchOnStart ? '🚀' : '💤'}
                </div>
